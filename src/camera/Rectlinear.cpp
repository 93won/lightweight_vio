/**
 * @file      Rectlinear.cpp
 * @brief     Implementation of Rectlinear (pinhole) camera model
...
#include "Rectlinear.h"

namespace lightweight_vio {

Rectlinear::Rectlinear(double fx, double fy, double cx, double cy, 
                       const std::vector<double>& distortion_coeffs)
    : Camera(fx, fy, cx, cy, distortion_coeffs)
{
}

cv::Point2f Rectlinear::undistort_point(const cv::Point2f& distorted_point) const {
    // Get camera matrix and distortion coefficients
    cv::Mat K = get_camera_matrix();
    cv::Mat D = get_distortion_mat();
    
    // Use OpenCV's undistortPoints (works for pinhole model)
    std::vector<cv::Point2f> distorted_pts = {distorted_point};
    std::vector<cv::Point2f> normalized_pts;
    
    cv::undistortPoints(distorted_pts, normalized_pts, K, D);
    ...
    const std::vector<cv::Point2f>& distorted_points) const 
{
    ...
    std::vector<cv::Point2f> normalized_pts;
    cv::undistortPoints(distorted_points, normalized_pts, K, D);
